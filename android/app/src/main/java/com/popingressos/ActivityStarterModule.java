...
final class ActivityStarterModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    void chamaCielo(@NonNull String urlCielo) {
        Activity activity = getCurrentActivity();
        if (activity != null) {
            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("lio://payment?request="+urlCielo+"&urlCallback=order://response"));
